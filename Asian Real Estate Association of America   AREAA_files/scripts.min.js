...
function userpro_shortcode_template( method, container, shortcode, up_username, force_redirect_uri, post_id ) {
	...
	jQuery.ajax({
		...
		success:function(data){
			...
			Recaptcha.create( form.parents('.userpro').data('recaptcha_publickey') , 'recaptcha_widget');
		}
	});
}
...
jQuery(".userpro-pic-upload").each(function(){
	...
	jQuery(this).uploadFile({
		...
		onSuccess:function(files,data,xhr){
			...
			var statusbar = jQuery('.ajax-file-upload-statusbar:visible');
			var src = statusbar.parents('.userpro').data('uploads_dir') + files[0];
			if (statusbar.parents('.userpro-input').find('img.default').length){
...
function userpro_chosen(){
	jQuery(".userpro select").removeClass("chzn-done").css('display', 'inline').data('chosen', null);
...
********
***********/
function userpro_collapse(){
	jQuery('.userpro-section').each(function(){
	...
	jQuery('.userpro-collapsible-1.userpro-collapsed-1').each(function(){
		jQuery(this).nextUntil('div.userpro-column').hide();
		if (jQuery(this).find('span').length==0) jQuery(this).prepend('<span><i class="userpro-icon-angle-right"></i></span>');
	});
	jQuery('.userpro-collapsible-1.userpro-collapsed-0').each(function(){
...
function userpro_side_validate( element, input_value, ajaxcheck ) {
	str = 'action=userpro_side_validate&input_value='+input_value+'&ajaxcheck='+ajaxcheck+'&element='+element;
	jQuery.ajax({
		url: userpro_ajax_url,
		data: str,
		dataType: 'JSON',
...
function userpro_client_error( element, parent, error) {
	if ( element.attr('type') ) {
		...
		if (parent.find('.userpro-warning').length==0) {
			...
			parent.find('.userpro-warning').animate({'top' : '0px', 'opacity' : '1'});
		} else {
...
function userpro_init_load(form) {
	...
	form.parents('.userpro').find('img.userpro-loading').show();
}
...
function userpro_end_load(form) {
	...
	form.parents('.userpro').find('img.userpro-loading').hide();
}
		...
		jQuery.ajax({
			...
			success:function(data){
				...
				if (data.error){
					...
					if (form.find('.userpro-recaptcha').length) {
					Recaptcha.reload();
					}
					...
					jQuery.each( data.error, function(key, value) {
						...
						if (key == 'recaptcha') {
							if (form.find('.userpro-recaptcha .userpro-input').find('.userpro-warning').length){
							form.find('.userpro-recaptcha .userpro-input').find('.userpro-warning').html(value);
							parent.find('.userpro-warning').animate({'top' : '0px', 'opacity' : '1'});
							} else {
							form.find('.userpro-recaptcha .userpro-input').append('<div class="userpro-warning"><i class="userpro-icon-caret-up"></i>' + value + '</div>');
							parent.find('.userpro-warning').animate({'top' : '0px', 'opacity' : '1'});
							}
						}
						...
						if (key == 'userpro_editor') {
							...
							form.find('.userpro-field-editor .userpro-input').find('.userpro-warning').animate({'top' : '0px', 'opacity' : '1'});
							} else {
				...
				userpro_ajax_picupload();
				userpro_collapse();
				userpro_overlay_center(container);
